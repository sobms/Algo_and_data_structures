...
clock_t time1, time2;
char* Create_str(struct Graph* graph) {
	struct Graph* ptr = graph;
	const char* str = "digraph graphname {";
	...
	while (ptr) { // create list of points
		s = strcat(strcat(s, ptr->top->name),";");
		ptr = ptr->next;
	}
	ptr = graph;
	while (ptr) {
		if (ptr->top->ribs) {
			...
			while (rib) {
				s = strcat(strcat(strcat(strcat(s, ptr->top->name),"->"),rib->top->name),";");
				rib = rib->next;
			}
		}
		ptr = ptr->next;
...
int Output_graph(struct Graph* graph) {
	char* str = Create_str(graph);
	Agraph_t* G = agmemread(str);
	GVC_t* gvc;
	gvc = gvContext();
	gvLayout(gvc, G, "dot");
...
void Deletion(struct Graph* graph,FILE* file = NULL) {
	...
	while (ptr) {
		prev = ptr;
		ptr = ptr->next;
		graph = delete_top(graph, prev->top, file);
	}
	

}
void Main_Menu(struct Graph* graph, int argc, char* argv[]) {
	int i = 0;
	char console_input;
	FILE* file = NULL;
